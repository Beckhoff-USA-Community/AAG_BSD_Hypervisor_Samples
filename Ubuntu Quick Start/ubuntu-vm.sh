...
vm_name="ubuntu-vm"
vm_dir="/home/Administrator/vms/ubuntu"
iso_cd0="${vm_dir}/ubuntu-installer.iso"
disk="/dev/zvol/zroot/ubuntu-vm_disk0"
...
vm::init(){
	...
	{ #Catch
		echo "vmm already loaded, skipping..."
	}
}
...
vm::run(){
	...
	while true; do
		...
		bhyve -c 2 -m 6G \
		-A -H \
		-l bootrom,/usr/local/share/uefi-firmware/BHYVE_BHF_UEFI.fd,${vm_dir}/EFI_VARS.fd,fwcfg=qemu \
		-s 0:0,hostbridge \
		-s 1:0,virtio-blk,${disk} \
